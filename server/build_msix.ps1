...
param(
    ...
    [string]$PublisherDisplayName = "Abbilaash",
...
if ($IncludeModels) {
    ...
    $optionalModelDirs = @("llama3", "whisper")
...
if (Test-Path "llama3") {
    Copy-Item -Path "llama3" -Destination $distPath -Recurse -Force
    Write-Host "Copied llama3" -ForegroundColor Green
}
if (Test-Path "whisper") {
    Copy-Item -Path "whisper" -Destination $distPath -Recurse -Force
    Write-Host "Copied whisper" -ForegroundColor Green
}
if (Test-Path "poppler") {
    Copy-Item -Path "poppler" -Destination $distPath -Recurse -Force
    Write-Host "Copied poppler" -ForegroundColor Green
}
